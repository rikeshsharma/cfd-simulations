...
function z=crankNicolson()
    ...
    nmax=tmax/deltat;
    for n=2:nmax
       phi(n)=(phi(n-1)+phid(n-1)*deltat-w2*deltat*deltat*phi(n-1)/4)/(1+w2*deltat*deltat/4);
       phid(n)=phid(n-1)-w2*(phi(n-1)+phi(n))*deltat/2;
    end
    subplot(3,2,2);
    z1=phi;
    t=linspace(0,tmax,tmax/deltat);
    plot(t,z1,'Color',[0.8500, 0.3250, 0.0980]);
    title('Crank Nicolson for \Deltat=0.1 sec');
    xlabel('t (sec)');
    ...
    title('Crank Nicolson for \Deltat=0.6 sec');
    xlabel('t (sec)');
    ...
    title('Crank Nicolson for \Deltat=2.1 sec');
    xlabel('t (sec)');
    ...
    title('Plots of solution for Exact and Crank Nicolson for different value of \Deltat');
    xlabel('t (sec)');
    ylabel('\Phi(t)');
